...
{	int j, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 6:	/* claim select */
		spin_c_typ[0] = 4; /* L_Fair */
		((P6 *)pptr(h))->c_cur[0] = 5;
		reached4[5]=1;
		spin_c_typ[1] = 3; /* L_Live */
		((P6 *)pptr(h))->c_cur[1] = 5;
		reached3[5]=1;
		((P6 *)pptr(h))->_t = 3;
		((P6 *)pptr(h))->_p = 5;
		((P6 *)pptr(h))->_n = 1; /* L_Live */
		src_claim = src_ln3;
		if (whichclaim == -1 && claimname == NULL)
			printf("0: Claim L_Live (3), from state 5\n");
		if (whichclaim != -1)
	...
	case 2:	/* WaitingRoom */
		...
		((P2 *)pptr(h))->_7_x = 0;
#ifdef VAR_RANGES
		logval("WaitingRoom:cid", ((P2 *)pptr(h))->_7_cid);
		logval("WaitingRoom:bid", ((P2 *)pptr(h))->_7_bid);
		logval("WaitingRoom:x", ((P2 *)pptr(h))->_7_x);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Customer */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("Customer:id", ((P1 *)pptr(h))->id);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Barber */
		...
		((P0 *)pptr(h))->_5_cid = 0;
#ifdef VAR_RANGES
		logval("Barber:id", ((P0 *)pptr(h))->id);
		logval("Barber:cid", ((P0 *)pptr(h))->_5_cid);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
	...
	{	int l_in;
		...
		now.nextc = 0;
		now.nnap = 0;
		now.to_room = addqueue(1, 1);
		now.to_cust[0] = addqueue(2, 1);
		now.to_cust[1] = addqueue(3, 1);
		now.to_cust[2] = addqueue(4, 1);
		now.to_barber[0] = addqueue(5, 1);
		now.to_barber[1] = addqueue(6, 1);
#ifdef VAR_RANGES
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("waiting[l_in]", now.waiting[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("napping[l_in]", now.napping[l_in]);
		}
	}
		logval("nwait", now.nwait);
		logval("nextc", now.nextc);
		logval("nnap", now.nnap);
#endif
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.to_room) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.to_room) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.to_room) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.to_room) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.to_cust[0]) )
			 ||     q_full((int) ( now.to_cust[1]) )
			 ||     q_full((int) ( now.to_cust[2]) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.to_cust[0]) )
			 ||    !q_full((int) ( now.to_cust[1]) )
			 ||    !q_full((int) ( now.to_cust[2]) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.to_cust[0]) )
			 ||     !q_len((int) ( now.to_cust[1]) )
			 ||     !q_len((int) ( now.to_cust[2]) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.to_cust[0]) )
			 ||      q_len((int) ( now.to_cust[1]) )
			 ||      q_len((int) ( now.to_cust[2]) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.to_barber[0]) )
			 ||     q_full((int) ( now.to_barber[1]) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.to_barber[0]) )
			 ||    !q_full((int) ( now.to_barber[1]) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.to_barber[0]) )
			 ||     !q_len((int) ( now.to_barber[1]) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.to_barber[0]) )
			 ||      q_len((int) ( now.to_barber[1]) )))
			    ) return 0; break;
