...
{	int j, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 9:	/* claim select */
		spin_c_typ[0] = 7; /* L_EventuallyServerBusy */
		((P9 *)pptr(h))->c_cur[0] = 5;
		reached7[5]=1;
		spin_c_typ[1] = 6; /* L_EventuallyOrder */
		((P9 *)pptr(h))->c_cur[1] = 5;
		reached6[5]=1;
		spin_c_typ[2] = 5; /* S_NeverSameOrder */
		((P9 *)pptr(h))->c_cur[2] = 5;
		reached5[5]=1;
		spin_c_typ[3] = 4; /* S_CashierCorrectOrder */
		((P9 *)pptr(h))->c_cur[3] = 5;
		reached4[5]=1;
		((P9 *)pptr(h))->_t = 4;
		((P9 *)pptr(h))->_p = 5;
		((P9 *)pptr(h))->_n = 3; /* S_CashierCorrectOrder */
		src_claim = src_ln4;
		if (whichclaim == -1 && claimname == NULL)
			printf("0: Claim S_CashierCorrectOrder (4), from state 5\n");
		if (whichclaim != -1)
	...
	case 2:	/* Server */
		...
		((P2 *)pptr(h))->id = par0;
		/* locals: */
		((P2 *)pptr(h))->_8_customer = 0;
		((P2 *)pptr(h))->_8_order = 0;
#ifdef VAR_RANGES
		logval("Server:id", ((P2 *)pptr(h))->id);
		logval("Server:customer", ((P2 *)pptr(h))->_8_customer);
		logval("Server:order", ((P2 *)pptr(h))->_8_order);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Cashier */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->_7_current_customer = 0;
		((P1 *)pptr(h))->_7_order = 0;
#ifdef VAR_RANGES
		logval("Cashier:current_customer", ((P1 *)pptr(h))->_7_current_customer);
		logval("Cashier:order", ((P1 *)pptr(h))->_7_order);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Customer */
		...
		reached0[9]=1;
		/* params: */
		((P0 *)pptr(h))->favorite = par0;
		((P0 *)pptr(h))->id = par1;
		/* locals: */
		((P0 *)pptr(h))->_6_order = 0;
#ifdef VAR_RANGES
		logval("Customer:id", ((P0 *)pptr(h))->id);
		logval("Customer:favorite", ((P0 *)pptr(h))->favorite);
		logval("Customer:order", ((P0 *)pptr(h))->_6_order);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
	...
	{	int l_in;
		...
		now.cust_order = addqueue(1, 1);
		now.cash_order = addqueue(2, 1);
		now.sever_order = addqueue(3, 1);
		now.server_to_cust[0] = addqueue(4, 1);
		now.server_to_cust[1] = addqueue(5, 1);
		now.server_to_cust[2] = addqueue(6, 1);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.favorite_foods[l_in] = 0;
		}
	}
		now.currentOrder = 0;
#ifdef VAR_RANGES
		logval("numCustWait", now.numCustWait);
		logval("currentCust", now.currentCust);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("cust[l_in]", now.cust[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("favorite_foods[l_in]", now.favorite_foods[l_in]);
		}
	}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.cust_order) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.cust_order) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.cust_order) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.cust_order) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.cash_order) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.cash_order) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.cash_order) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.cash_order) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.sever_order) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.sever_order) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.sever_order) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.sever_order) )))
			    ) return 0; break;
		case 4: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.server_to_cust[0]) )
			 ||     q_full((int) ( now.server_to_cust[1]) )
			 ||     q_full((int) ( now.server_to_cust[2]) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.server_to_cust[0]) )
			 ||    !q_full((int) ( now.server_to_cust[1]) )
			 ||    !q_full((int) ( now.server_to_cust[2]) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.server_to_cust[0]) )
			 ||     !q_len((int) ( now.server_to_cust[1]) )
			 ||     !q_len((int) ( now.server_to_cust[2]) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.server_to_cust[0]) )
			 ||      q_len((int) ( now.server_to_cust[1]) )
			 ||      q_len((int) ( now.server_to_cust[2]) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	byte   numCustWait:	%d\n", now.numCustWait);
	printf("	byte   currentCust:	%d\n", now.currentCust);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	byte   cust[%d]:	%d\n", l_in, now.cust[l_in]);
		}
	}
	printf("	chan cust_order (=%d):	len %d:\t", now.cust_order, q_len(now.cust_order));
	c_chandump(now.cust_order);
	printf("	chan cash_order (=%d):	len %d:\t", now.cash_order, q_len(now.cash_order));
	c_chandump(now.cash_order);
	printf("	chan sever_order (=%d):	len %d:\t", now.sever_order, q_len(now.sever_order));
	c_chandump(now.sever_order);
	printf("	chan server_to_cust[0] (=%d):	len %d:\t", now.server_to_cust[0], q_len(now.server_to_cust[0]));
	c_chandump(now.server_to_cust[0]);
	printf("	chan server_to_cust[1] (=%d):	len %d:\t", now.server_to_cust[1], q_len(now.server_to_cust[1]));
	c_chandump(now.server_to_cust[1]);
	printf("	chan server_to_cust[2] (=%d):	len %d:\t", now.server_to_cust[2], q_len(now.server_to_cust[2]));
	c_chandump(now.server_to_cust[2]);
}
void
...
{	/* int i; */
	...
	printf("	byte   id:	%d\n", ((P2 *)pptr(pid))->id);
	printf("	byte   customer:	%d\n", ((P2 *)pptr(pid))->_8_customer);
		break;
	case 1:
		printf("local vars proc %d (Cashier):\n", pid);
	printf("	byte   current_customer:	%d\n", ((P1 *)pptr(pid))->_7_current_customer);
		break;
	case 0:
...
printm(int x)
{
	...
	case 2: Printf("PIZZA"); break;
	case 3: Printf("SANDWICH"); break;
	case 4: Printf("CHILI"); break;
	default: Printf("%d", x);
	}
}
void
...
{	uchar *z; int slot;
	...
	case 6:
		...
		{	printf(" [");
			printm(((Q6 *)z)->contents[slot].fld0);
			printf("],");
		}
		break;
	case 5:
